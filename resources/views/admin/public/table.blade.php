                ...
                switch (value.event) {
                    ...
                    case 'LAY_TABLE_DELETE': {
                        layer.confirm('{{ trans('admin.table.delete') }}: ' + value.data.id, async function (index) {
                            ...
                            if (!response.ok) {
                                // popup.failure(`{{ trans('admin.table.delete') . trans('admin.word.gap') .  trans('admin.status.failure') }}: ${response.status}: ${response.statusText}`);
                            ...
                            if (code || msg) {
                                popup.failure(`{{ trans('admin.table.delete') . trans('admin.word.gap') .  trans('admin.status.failure') }}: ${msg}`);
                                return false;
                            }

                            popup.success('{{ trans('admin.table.delete') . trans('admin.word.gap') .  trans('admin.status.success') }}', function() {
                                // remove row dom
                                value.del();
