    ...
    <script>
        layui.use(['admin', 'popup', ], function () {
            ...
            layui.form.on('submit(form-submit)', async function (data) {
                ...
                console.log(data);
                console.log(field);
                console.log(layui.jquery('form').serializeObject());
                ...
                if (!response.ok) {
                    // popup.failure(`{{ $name . trans('admin.word.gap') . trans('admin.status.failure') }}: ${response.status}: ${response.statusText}`);
                ...
                if (code || msg) {
                    popup.failure(`{{ $name . trans('admin.word.gap') . trans('admin.status.failure') }}: ${msg}`);
                    return false;
                }

                popup.success('{{ $name . trans('admin.word.gap') . trans('admin.status.success') }}', function() {
                    // get:    /path            => index
