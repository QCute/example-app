                        ...
                        <div class="form-center-box">
                            ...
                            <form style="margin-top: 30px;" class="layui-form" action="return false;">
                                ...
                                <div class="tab-log-verification">
                                    ...
                                    <img src="{{ admin_asset("./admin/images/captcha.gif") }}" alt="" class="verification-img" />
                                </div>
                                <div class="layui-form-item">
    ...
    <script>
        layui.use(['form', 'button', 'popup'], function () {
            ...
            form.on('submit(login)', async function (data) {
                ...
                const url = "{{ '/' . config('admin.route.prefix') . '/auth/login' }}";
                ...
                if (!response.ok) {
                    popup.failure(`{{ trans('admin.login') . trans('admin.word.gap') . trans('admin.status.failure') }}: ${response.status}: ${response.statusText}`);
                    return false;
                }
                ...
                if (code || msg) {
                    popup.failure(`{{ trans('admin.login') . trans('admin.word.gap') . trans('admin.status.failure') }}: ${code || msg}`);
                    return false;
                }

                popup.success('{{ trans('admin.login') . trans('admin.word.gap') . trans('admin.status.success') }}', function () {
                    location.href = '{{ '/' . config('admin.route.prefix') }}';
                });
