                ...
                <ul class="layui-nav layui-layout-right">
                    ...
                    <li class="layui-nav-item user">
                        ...
                        <dl class="layui-nav-child">
                            <dd>
                                <a user-menu-url="{{ '/' . config('admin.route.prefix') . config('admin.auth.profile') }}" user-menu-id="" user-menu-title="{{ trans('admin.profile') }}">
                                    {{ trans('admin.profile') }}
                                </a>
                            </dd>
                            <dd>
        ...
        <script>
            ...
            layui.use(['admin', 'menu', 'jquery', 'popup', 'page', 'nprogress'], function() {
                ...
                $(document).on("ajaxSend", function(event, state, config) {
                    ...
                    if(login) {
                        throw location.href = "{{ '/' . config('admin.route.prefix') . config('admin.auth.redirect_to') }}";
                    }
                ...
                // yml | json | api
                const path = "{{ '/' . config('admin.route.prefix') . config('admin.vendor.config.url') }}";
                ...
                admin.logout(async function() {
                    ...
                    const url = "{{ '/' . config('admin.route.prefix') . '/auth/logout' }}";
                    await fetch(url, {
                        method: 'GET',
                        headers: {
                    ...
                    popup.success('{{ trans('admin.logout') . trans('admin.word.gap') . trans('admin.status.success') }}', function() {
                        location.href = "{{ '/' . config('admin.route.prefix') . config('admin.auth.redirect_to') }}";
                    });
                ...
                form.on('select(channel-select-filter)', async function(data) {
                    ...
                    // request
                    const url = "{{ '/' . config('admin.route.prefix') . '/channel/change' }}";
                ...
                form.on('select(server-select-filter)', async function(data) {
                    // request
                    const url = "{{ '/' . config('admin.route.prefix') . '/server/change' }}";
                    const response = await fetch(url, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
