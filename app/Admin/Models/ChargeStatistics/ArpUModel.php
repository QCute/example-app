...
class ArpUModel extends DistributionModel 
{
    /**
        ...
        for($date = $begin; $date < $end; $date += 86400) {
            ...
            foreach($range as $day) {
                ...
                $sub = $model
                    ...
                    ->select([
                        // DB::raw("CONCAT(IFNULL(SUM(`money`), 0), '/', COUNT(DISTINCT login_log.`role_id`), '(', FORMAT(IFNULL(IFNULL(SUM(`money`), 0) * 100 / COUNT(DISTINCT login_log.`role_id`), 0), 2), '%', ')') AS `day_$day`"),
