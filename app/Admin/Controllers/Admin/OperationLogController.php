...
class OperationLogController extends Controller 
{
    public function index(Request $request)
    {
        ...
        $header = [
            ...
            (new Header())->field('user_id')->title(trans('admin.user') . trans('admin.word.gap') . trans('admin.id'))->align()->width(160),
    ...
    public function show(Request $request, int $id)
    {
        ...
        $form
            ->display('user_id')
            ->label(trans('admin.user') . trans('admin.word.gap') . trans('admin.id'))
            ->value($data->user_id);
        $form
            ->display('path')
