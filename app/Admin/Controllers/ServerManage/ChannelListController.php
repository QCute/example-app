...
class ChannelListController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $columns = Schema::connection('extend')->getColumns(config('extend.database.channel_table'));
    ...
    public function edit(Request $request, int $id)
    {
        ...
        $roleChannels = collect($data->roles)->keyBy('role_id');
    ...
    public function update(Request $request, int $id)
    {
        ...
        try {
            ...
            $update = [];
            foreach($roles as $role) {
                if(!$channel->roles->contains($role->id)) {
                    $update[] = $role;
                }
            }
            ...
            RoleChannelModel::upsert($update, ['role_id', 'channel_id']);

            $channel->permission = !empty($update) ? 'ROLE' : '';
            $channel->save();

            $destroy = [];
            foreach($channel->roles as $roleChannel) {
                if(!$roles->contains($roleChannel->role_id)) {
                    $destroy[] = $roleChannel;
                }
            }

            $destroy = collect($destroy)->map(function($roleChannel) {
                return $roleChannel->id;
            });

            RoleChannelModel::destroy($destroy);
