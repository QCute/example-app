...
class ServerTuningController extends Controller
{
    public function index(Request $request)
    {
        ...
        // owner lock check
        ['owner' => $owner] = $this->getServerLockOwner($request);
        if ((!is_null($owner) || $owner !== '') && $owner != Auth::user()->name) {

            $form->display('')->default("This server lock by {$owner}");

            return $form->name(trans('admin.form.show'))->build();
        }

        $form->display('get-local-time')->label(trans('admin.local') . trans('admin.word.gap') . trans('admin.time'));

        $form->dateTime('set-server-time')->label(trans('admin.server') . trans('admin.word.gap') . trans('admin.time'));

        $form->display('get-server-time')->label(trans('admin.server') . trans('admin.word.gap') . trans('admin.time'));

        $form->display('get-server-state')->label(trans('admin.server') . trans('admin.word.gap') . trans('admin.state'));
...
HTML);
    ...
    public function setServerOpenTime(Request $request)
    {
        $time = $request->input("time");
        $server = ServerService::getServer();
        $msg = MachineService::execute($server, ["cfg", "set", $server->node, "main, open_time", strtotime($time)]);
        return ['code' => 0, 'msg' => $msg];
    }
