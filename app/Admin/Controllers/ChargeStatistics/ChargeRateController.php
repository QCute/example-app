...
use App\Admin\Models\ChargeStatistics\ChargeRateModel;
use Illuminate\Http\Request;

class ChargeRateController extends StatisticsController
{
    public function index(Request $request)
    {
        ...
        $range = range(2, 30);
        $data = ChargeRateModel::getDaily($begin, $end, $range);

        $header = collect([
            (new Header())->field('date')->title(trans('admin.time.date'))->width(120)->align(),
            (new Header())->field('number')->title(trans('admin.statistics.number'))->width(120)->align(),
        ]);
