...
class SeederExporter extends Command
{
    ...
    protected function replace(string $name, string $className, array $exceptFields = [], string $contents = '')
    {
        ...
        foreach($list as $key => $value) {
            ...
            // not table
            if(substr($key, -5) != 'table') continue;

            // not log table
            if(substr($key, -9, 3) == 'log') continue;

            // not user and user_role table
            if(substr($key, 0, -6) == 'user' || substr($key, 0, -6) == 'user_role') continue;

            $table = $list[$key];

            $key = substr_replace($key, '_model', -6);
            $model = $list[$key];
